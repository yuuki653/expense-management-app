import React, { useState, useMemo } from "react";
import { useNavigate } from "react-router-dom";
import AddCategory from "../components/AddCategory";

import { Category, Expense } from "../types/index";

interface CategoryManagementPageProps {
  setCategories: ([]: Category[]) => void;
  categories: Category[];
  expenses: Expense[];
}

const CategoryManagementPage: React.FC<CategoryManagementPageProps> = ({
  setCategories,
  categories,
  expenses,
}) => {
  const navigate = useNavigate();

  return (
    <div className="w-[90%] mx-auto">
      <div className="flex items-center justify-center my-5">
        <hr className="flex-grow border-2 border-gray-400" />
        <p className="mx-3 text-lg font-bold">category management</p>
        <hr className="flex-grow border-2 border-gray-400" />
      </div>
      <div className="w-60 mx-auto mt-10">
        <button
          className="flex text-gray-600 hover:underline decoration-2 underline-offset-4 text-sm hover:text-gray-700 transition-colors"
          onClick={() => {
            navigate(-1);
          }}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            fill="currentColor"
            className="size-4"
          >
            <path
              fillRule="evenodd"
              d="M12.5 9.75A2.75 2.75 0 0 0 9.75 7H4.56l2.22 2.22a.75.75 0 1 1-1.06 1.06l-3.5-3.5a.75.75 0 0 1 0-1.06l3.5-3.5a.75.75 0 0 1 1.06 1.06L4.56 5.5h5.19a4.25 4.25 0 0 1 0 8.5h-1a.75.75 0 0 1 0-1.5h1a2.75 2.75 0 0 0 2.75-2.75Z"
              clipRule="evenodd"
            />
          </svg>
          back
        </button>
      </div>
      <AddCategory
        categories={categories}
        setCategories={setCategories}
        expenses={expenses}
      />
    </div>
  );
};

export default CategoryManagementPage;
